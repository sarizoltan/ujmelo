...
if (isset($_GET['status']) && isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if (in_array($_GET['status'], $allowed) && csrf_verify()) {

        $booking_id = (int)$_GET['id'];
        $new_status = $_GET['status'];

        // Foglalás lekérése email küldéshez
        $stmt = $pdo->prepare("
            SELECT b.*, s.name as staff_name,
                   sv.name as service_name, sv.duration as service_duration, sv.price as service_price
            FROM bookings b
        ...
        if ($booking) {
            ...
            // Email küldés ha változott a státusz
            if ($old_status !== $new_status) {
                booking_status_email($booking, $new_status);
            }
...
$stmt = $pdo->prepare("
    SELECT b.*, s.name as staff_name, sv.name as service_name, sv.duration as service_duration
    FROM bookings b
    JOIN staff s    ON b.staff_id   = s.id
    JOIN services sv ON b.service_id = sv.id
        ...
        <form method="POST" action="bookings.php">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save"></i> Foglalás létrehozása
                </button>
            </div>
        </form>
    </div>
...
function booking_status_email(array $booking, string $new_status): void
{
    ...
    $date_hu   = date('Y. m. d.', strtotime($booking['booking_date']));
    $price_fmt = isset($booking['service_price'])
        ? number_format((float)$booking['service_price'], 0, ',', ' ') . ' Ft'
        : '';

    $configs = [
        'confirmed' => [
            'subject' => "✅ Foglalásod megerősítve – {$booking['booking_ref']}",
        ...
        'cancelled' => [
            'subject' => "❌ Foglalás lemondva – {$booking['booking_ref']}",
            'title'   => '❌ Foglalásod lemondásra került',
            'color'   => '#ef4444',
            'icon'    => '❌',
            'intro'   => "Kedves <strong>{$booking['customer_name']}</strong>!<br><br>
                          Sajnálattal értesítünk, hogy foglalásod <strong>lemondásra került</strong>.",
            'extra'   => "<div class='info-box' style='border-color:#ef4444;background:#fef2f2;color:#991b1b;'>
                            Ha kérdésed van, kérjük vedd fel velünk a kapcsolatot!<br>
                            " . ($site_phone ? "📞 <a href='tel:{$site_phone}'>{$site_phone}</a><br>" : '') . "
                            " . ($site_email ? "✉️ <a href='mailto:{$site_email}'>{$site_email}</a>" : '') . "
                          </div>
                          <div class='cta'>
                            <a href='" . BASE_URL . "/foglalas'>Új időpont foglalása →</a>
                          </div>",
        ],
        'completed' => [
            ...
            'intro'   => "Kedves <strong>{$booking['customer_name']}</strong>!<br><br>
                          ...
                          Reméljük elégedett voltál a szolgáltatással. 😊",
            'extra'   => "<div class='cta'>
                            <a href='" . BASE_URL . "/foglalas'>Foglalj ismét →</a>
                          </div>",
        ],
    ];
...
<style>
  ...
  .hdr{background:#1a1a1a;padding:28px 32px;text-align:center;}
  .hdr .ico{font-size:42px;display:block;margin-bottom:10px;}
...
<div class='wrap'>
  ...
  <div class='bdy'>
    <p style='font-size:15px;line-height:1.7;'>{$cfg['intro']}</p>
    <div class='ref-box'>
    ...
    <div class='details'>
      ...
      " . ($price_fmt ? "<div class='dr'><span class='dl'>Ár</span><span class='dv'>{$price_fmt}</span></div>" : '') . "
    </div>
    {$cfg['extra']}
  </div>
  <div class='ftr'>
    ...
    <p style='margin-top:10px;'>© " . date('Y') . " {$site_name}</p>
  </div>
</div>
</body></html>";

   send_mail($booking['customer_email'], $cfg['subject'], $body);
